...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(!preg_match("/^[0-9]*$/",$_POST['person_id'])) {
...
if ($valid == true) {
  ...
  $sql = "SELECT * FROM person WHERE 'person_id' = '$_POST[person_id]' AND 'role' = 'S'";
  ...
  if($data) {
      ...
      $sql = "INSERT INTO `person`(`person_id`, `person_fName`, `person_lName`, `person_street1`, `person_street2`, `person_city`, `person_state`, `person_zipcode`, `person_phone`, `person_email`, `role`) VALUES ('$teacher_id', '$teacher_fName', '$teacher_lName', '$teacher_street1', '$teacher_street2', '$teacher_city', '$teacher_state', '$teacher_zipcode', '$teacher_phone', '$teacher_email', 'T')";
...
if ($valid == true) {
  ...
  $sql = "SELECT * FROM person WHERE 'person_id' = '$_POST[teacher_id]' AND 'role' = 'S'";
  $rs = mysqli_query($conn, $sql);
  $data = mysqli_fetch_array($rs, MYSQLI_NUM);
  if($data) {
      echo "<script type='text/javascript'>alert('teacher id already exists');</script>";
